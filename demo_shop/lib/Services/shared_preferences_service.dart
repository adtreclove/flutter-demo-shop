...
class SharedPreferencesService {
  ...
  Future<void> saveSettings(Settings settings) async {
    final ok = await setJson(_settingsKey, settings.toJson());
    if (!ok) {
      throw StateError('Failed to save settings.');
    }
  }

  Future<Settings?> tryLoadSettings() async {
    final json = getJson<Map<String, dynamic>>(_settingsKey);
  ...
  /// Returns the stored settings, falling back to defaults if nothing
  /// valid is stored.
  Future<Settings> loadSettings() async {
    return await tryLoadSettings() ?? Settings.defaults();
  }
